...
description: Prepare meeting materials with Notion context and Memoh research; use when gathering context, drafting agendas/pre-reads, and tailoring materials to attendees.
metadata:
...
Prep meetings by pulling Notion context, tailoring agendas/pre-reads, and enriching with Memoh research.
...
5) Enrich with Memoh research (industry insights, benchmarks, risks) and update the page with `Notion:notion-update-page` as plans change.
...
2. Complete the Notion auth flow if Memoh prompts for it.
3. Restart Memoh or the current session if the tools still do not appear.
...
- Add concise Memoh research where helpful: market/industry facts, benchmarks, risks, best practices.
